...
if [[ -f "prisma/schema.prisma" ]]; then
    ...
    # Migracik alkalmazasa (ha vannak)
    if [[ -d "prisma/migrations" ]]; then
        log_info "Prisma migrate deploy futtatasa..."
        if node node_modules/prisma/build/index.js migrate deploy 2>&1; then
            log_success "Migracik alkalmazva"
        else
            log_warning "Prisma migrate sikertelen (lehet hogy nincs uj migracio)"
        fi
    else
        log_info "Nincs migrations konyvtar - db push hasznalata"
...
# Minden 3000-es porton futo folyamat leallitasa (root vagy barki mas)
PIDS_3000=$(lsof -t -i :3000 2>/dev/null || true)
if [[ -n "$PIDS_3000" ]]; then
    log_info "Port 3000-en futo folyamatok: $PIDS_3000"
    echo "$PIDS_3000" | xargs -r kill -9 2>/dev/null || true
    sleep 2
    log_success "Regi folyamatok leallitva"
else
    log_info "Nincs futo folyamat a 3000-es porton"
fi
...
if [[ -f "ecosystem.config.js" ]]; then
    if pm2 list | grep -q "matrixcbs"; then
        pm2 reload ecosystem.config.js --env production
        log_success "PM2 ujrainditva (reload)"
    else
        pm2 start ecosystem.config.js --env production
        pm2 save
        log_success "PM2 elinditve"
    fi
else
    log_warning "ecosystem.config.js nem talalhato!"
...
EOF
    ...
    pm2 start ecosystem.config.js --env production
    pm2 save
    log_success "PM2 elinditve uj konfiggal"
fi
